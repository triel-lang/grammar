...
### 2.11 Grammar Ambiguity and Consistency Fixes

Sections 2.1 through 2.10 addressed semantics and cryptography; this section addresses the grammar's own internal consistency — twelve findings, most flagged as structural ambiguities the reference parser published alongside this grammar (`parser/`) can now confirm are gone rather than merely argue are gone.
...
`parser/test_ambiguity.py`, a set of fragments chosen to stress the
constructs these fixes touch, six of which had multiple derivations
before F-6, together with inputs that must be rejected; it runs in CI
on every push.
...
**Writing negative numbers where a bare literal is expected (D-24).** `integer_literal` and `float_literal` had no sign; the only way to write a negative value was the general subtractive `unary_expr`, an expression-level construct not available wherever a bare `literal` or `numeric_literal` is expected directly — `THRESHOLD`, `SENSITIVITY_BOUND`, `TOLERANCE`, `WEIGHT`, `POLARITY`, and comparisons inside a `zk_constraint`. The grammar's own comment on `caused_by_entry` documented `WEIGHT ∈ [-1.0, 1.0]`, a range that was not writable. A new `signed_numeric_literal` (and, for the ZK case, a parallel `signed_literal`) production is added at exactly these bare positions — deliberately *not* by making `numeric_literal` itself signed, since `numeric_literal` also feeds `expr` through `literal`/`primary`, and doing so there would have reopened a D-19-shaped ambiguity: `PENALTY -50` would have gained two derivations, one through `unary_expr`'s own minus and one through a pre-signed literal. That interaction was caught during testing and the fix scoped accordingly, rather than shipped with the new ambiguity intact.

**Fixing a self-contradictory constraint (D-25).** `QUORUM_THRESHOLD`'s stated default (5) and its stated constraint (1 ≤ QUORUM_THRESHOLD ≤ n, where n is the number of declared modalities) were jointly unsatisfiable whenever a specification omitted `MODALITIES` — n would be 0, and the default of 5 would violate the constraint on every such specification, which is most of them. The constraint is now explicitly scoped to specifications that declare `MODALITIES` at all; its absence means the field is accepted but unchecked.

**Making named terms usable (D-26).** `named_term_decl` (`identifier := term_stmt`) let a specification name a term but no production referenced that name back, so the syntax could be written but never used. A `term_ref` alternative in `matched_term`, written `REF name`, closes the loop, resolved at semantic analysis the same way `contract_ref_stmt`'s `EXECUTE` target and `modality_id` already are. The `REF` keyword is required: a bare identifier would be ambiguous wherever an expression ends a term — in `alice MAY pay WHEN a OR b`, `b` could be a factor in the guard or a reference to a term named `b`, and `alice ON_BREACH PENALTY a OR b` has the same two readings.

**Removing an undefined construct (D-27).** `state_invariant`'s `DAG(identifier)` form had no semantics defined anywhere — not in this grammar, not in the technical report, not in the SEMANTIC RULES section. A construct that parses but means nothing is worse than one that doesn't parse, since it looks like a real, checkable property; it has been removed rather than retroactively defined, since inventing a meaning for it now would be exactly the kind of unfounded claim this document's revision history has been trying to eliminate.

**Supporting non-ASCII text (D-28).** `string_literal`'s `character` production was restricted to ASCII with no escape mechanism — disqualifying for a language whose own examples target the EU: a party's legal name in German, French, Greek, or Cyrillic script was not writable, and a literal double-quote could not appear inside a string at all. `character` now admits any Unicode code point directly, plus a conventional escape set (`\"`, `\\`, `\n`, `\t`, `\uXXXX`). `identifier` is deliberately left ASCII-only — keyword matching (Section 2, reserved list) depends on a fixed ASCII vocabulary, and the practical need this finding raised goes through string-literal contexts (`JURISDICTION`, `STANDARD`, `DID`), not identifiers.

**Documenting what remains unchecked (D-30).** `invariant_type` (`SAFETY`/`LIVENESS`/`FAIRNESS`) is, and remains, purely declarative: nothing checks that a formula tagged `SAFETY` doesn't actually express a liveness property. Classifying a temporal formula's actual shape requires a tool that understands temporal logic well enough to do so — the same category of analysis this report has repeatedly deferred to future work. The grammar's comment on `invariant_type` now says this plainly rather than leaving the label's authority ambiguous.

**Allowing trailing separators (D-31, partial).** `SUBJECTS`, `FACTORS`, `MODALITIES`, and `CROSS_MODAL` (comma-delimited) and `TERMS` and `INVARIANTS` (semicolon-delimited) each now accept one optional trailing separator before their closing brace, so adding a member no longer requires editing the line above it. What this does not fix, and does not claim to: the two delimiter families themselves remain genuinely different across block kinds, exactly as the finding described. Unifying them would be a breaking change to every specification using this grammar, not only to this repository's examples, and is left open for a future major version rather than forced through here.
