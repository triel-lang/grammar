...
**Fixing a self-contradictory constraint (D-25).** `QUORUM_THRESHOLD`'s stated default (5) and its stated constraint (1 ≤ QUORUM_THRESHOLD ≤ n, where n is the number of declared modalities) were jointly unsatisfiable whenever a specification omitted `MODALITIES` — n would be 0, and the default of 5 would violate the constraint on every such specification, which was three of this repository's own four examples. The constraint is now explicitly scoped to specifications that declare `MODALITIES` at all; its absence means the field is accepted but unchecked.

**Making named terms usable (D-26).** `named_term_decl` (`identifier := term_stmt`) let a specification name a term but no production referenced that name back, so the syntax could be written but never used. A new `term_ref` alternative in `matched_term` closes the loop, resolved at semantic analysis the same way `contract_ref_stmt`'s `EXECUTE` target and `modality_id` already are.
